...
-- Fully idempotent — safe to re-run this whole file anytime, even if some
-- of it already exists.

-- `domains` table backing SupabaseKnowledgeStore.
...
create policy "poc demo — allow all" on domains for all using (true) with check (true);

insert into domains (slug, name, description, typical_modules, seed_questions)
values
  ('booking_platform', 'Booking Platform',
   'A service where customers schedule appointments or reservations with providers.',
...
-- `projects` table: persists completed 5-document packages so they
-- survive server restarts and power the Dashboard/History view.
