...
def learn_domain(idea_text: str, proposed_name: str, project_id: str, store=None) -> dict:
    ...
    log_agent_call(logger, project_id, "knowledge_engine", "started", {"step": "learn_domain", "proposed_name": proposed_name})
    client = get_client()
    domain_data = _mock_learn(idea_text, proposed_name) if client is None else _live_learn(client, idea_text, proposed_name)
    slug = _slugify(domain_data.get("name", proposed_name))
    store.save_domain(slug, domain_data)
    log_agent_call(logger, project_id, "knowledge_engine", "completed", {"step": "learn_domain", "domain_slug": slug})
...
if __name__ == "__main__":
    ...
    result = learn_domain("A tool that helps small farms track crop yields", "agriculture_management", "demo-1", store)
